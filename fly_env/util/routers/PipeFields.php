 ...
 */
 ...
 abstract class PipeFields {
    ...
    private function set_fml($router)
    {
        if(array_key_exists('@fml',$router)) {
            if(is_bool($router['@fml'])) {
                self::$has_fml = $router['@fml'];
                if(self::$has_fml) {
                    fopen(FLY_ENV_STATIC_FML_PORTAL_PATH,'w');
                    $this->has_static = true;
                }
            } else {
                throw new \Exception('FML mode in router '.$this->route_index.'in '.$this->request_method.'.json must be set to a boolean type');
            }
        }
    }
 }