...
function migrate_models($argv_length,$general_path)
{
    try{
        ...
        } else throw new Exception(">> fly-env: migrate_models: command is an empty type. No preceeding arguments needed");
    } catch(Exception $err) {
...
function make_model($cmd, $general_path)
{
    try {
        ...
        } else throw new Exception(">> fly-env: make_model: command expects model name.\nHINT: make_model [Model NAME HERE].");
    } catch(Exception $err) {
        echo PHP_EOL.$err->getMessage().PHP_EOL.PHP_EOL;
    }
}
...
function createModel($modelName, $general_path)
{
    ...
    if(!file_exists("{$general_path}app/models/$modelName/handlers.sql")) {
        ...
        fwrite($file,"-- Write your routines and proceedures here\n");
        fclose($file);
    }
}